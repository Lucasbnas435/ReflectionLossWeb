...
def RL_dinamico_plot(csv_filename, txt_filename, undfrequencia, espamostra, baixar_grafico, hash_arquivo, coaxial=False):
	arq = open(f'./pythonPaginaINPE/static/files/txt_gerado/{txt_filename}','r')
	ler = arq.readlines()
	arq.close()

	#Ajuste da Referencia de L1 e L2
	d = float(espamostra) #[m] #Espessura da amostra (Livro chama de L)
	...
	'<input type="range" id="espamostra" name="espamostra" min={{inicio_slider * 1e-3}} max={{fim_slider * 1e-3}} step="0.000001" value={{espamostra}} onchange=submit()>' + \
	'<label for="espamostra">Espessura: {{espamostra}} m</label>' + \
	'</form>' + \
	f'<form action="{rota_grafico}" method="post">' + \
	...
	with open(f'./pythonPaginaINPE/templates/{nome_template}','w') as html_file:
