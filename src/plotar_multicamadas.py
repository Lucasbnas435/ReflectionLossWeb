...
from baixar_grafico_mult import baixar_grafico_multicamadas
from menos_pontos_mult import plotar_menos_pontos_multicamadas
...
def multicamadas_plot(hash_arquivo1, hash_arquivo2, hash_arquivo3, espamostra, csv_filename1, csv_filename2, csv_filename3, baixar_grafico, menos_pontos, divisor_quantidade_pontos=25):
    #Cuidado com a Sequência das amostras: a amostra encostada na placa deve ser a PRIMEIRA (índice = 0)
    TXT = [f"{hash_arquivo1}.txt", f"{hash_arquivo2}.txt", f"{hash_arquivo3}.txt"]
    ...
    if baixar_grafico:
        filename = baixar_grafico_multicamadas(samples, espamostra, hash_arquivo1, F, F_grafic, S11)
        return filename
    elif menos_pontos:
        filename = baixar_grafico_multicamadas(samples, espamostra, hash_arquivo1, F, F_grafic, S11)
        template_name = plotar_menos_pontos_multicamadas(samples, filename, hash_arquivo1, divisor_quantidade_pontos)
        return template_name

    # ------------------------GRAFICO 1 - Reflection Loss (RL)-----------------------
    fig = plt.figure(figsize=(10, 5))
    plt.plot(F_grafic, S11, 'r-', label="Espessura total = %.2f mm" % ((samples[0].d+samples[1].d+samples[2].d)*1000), alpha=0.4)
    plt.title(f"Camadas: {csv_filename1} + {csv_filename2} + {csv_filename3} + Placa")
	...
	'<form action="/multicamadas" method="post">' + \
    ...
    '<input type="hidden" name="hash_arquivo3" value={{hash_arquivo3}}>' + \
    '<input type="hidden" name="csv_filename1" value={{csv_filename1}}>' + \
    '<input type="hidden" name="csv_filename2" value={{csv_filename2}}>' + \
    '<input type="hidden" name="csv_filename3" value={{csv_filename3}}>' + \
    ...
    '<input type="range" id="espessura_amostra3" name="espessura_amostra3" min={{inicio_slider * 1e-3}} max={{fim_slider * 1e-3}} step="0.000001" value={{espessura_amostra3}} onchange=submit()>' + \
    '<label for="espessura_amostra3">Espessura 3: {{espessura_amostra3}} m</label><br>' + \
	'</form>' + \
    '<form action="/multicamadas" method="post">' + \
    ...
    '<input type="hidden" id="espessura_amostra3" name="espessura_amostra3" value={{espessura_amostra3}}>' + \
    '<input type="hidden" name="baixar_grafico" value="1">' + \
    '<input type="hidden" name="menos_pontos" value="0">' + \
    '<button type="submit">Baixar dados do gráfico</button>' + \
    '</form>' + \
    '<form action="/menospontosmult" method="post">' + \
    '<input type="hidden" name="inicio_slider" value={{inicio_slider}}>' + \
	'<input type="hidden" name="fim_slider" value={{fim_slider}}>' + \
	'<input type="hidden" name="hash_arquivo1" value={{hash_arquivo1}}>' + \
    '<input type="hidden" name="hash_arquivo2" value={{hash_arquivo2}}>' + \
